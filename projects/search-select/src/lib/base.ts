...
type Searches = Observable<OptionEntry | string | null>;
...
export class SearchSelectBase
  ...
  focus() {
    ...
    this.selectedValueSub = this.selectedValue.subscribe(
      this.checkAndPropagate.bind(this)
    );
  }
  ...
  blur() {
    ...
    if (this.selectedValueSub) {
      ...
      this.selectedValueSub = undefined;
    }
  ...
  private selectedValueSub: Subscription | undefined = undefined;
  private selectedValue: Observable<any>;

  constructor() {
    const searches: Searches = this.searchControl.valueChanges.pipe(
    ...
    this.selectedValue = options.pipe(
      ...
      map(([result, ds]) => {
        ...
        const entry = list.find(option =>
          matchFn(result.search, option)
        );
    ...
    this.empty = options.pipe(
      map(o => (o.list ? o.list.length === 0 : false))
    );
    ...
    this.incomingDataSourcesSub = this.incomingValues
      .pipe(
        withLatestFrom(this.incomingDataSources),
        switchMap<[any, DataSource], Observable<OptionEntry | null>>(
          ([value, ds]) => ds.displayValue(value)
        )
      )
